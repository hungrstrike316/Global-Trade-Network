...
if ~exist('kurParams','var')
    ...
    kurParams.sigW = w_spread(wi)*kurParams.muW; % STD of oscillator resonant phase distribution (but all run thru abs function after)
    kurParams.Kscale = Ks(ki);                    % scaling on network weights for the kuramoto simulation (an arbitrary parameter over which we grid search)
    %
